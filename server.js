...
app.post('/recharge', async (req, res)=> {
  ...
  instance.orders.create(options,async function (err, order) {
    ...
    if (!err) {
      return res.status(200).json({ order, key: process.env.RAZ_KEY, email: req.body.email, url: process.env.APP_URL + "/response" });
...
app.post('/response',async function (req, res) {
  ...
  const recharging = await Recharging.findOne({order:req.body.razorpay_order_id}).catch(err => {
